...
3.  The use of base graphics may not match other figures in a project that uses Ggplot2.

The Stylo2gg package, now at version 1.0, has been shared to address these concerns. See [github.com/jmclawson/stylo2gg](https://github.com/jmclawson/stylo2gg) for the code, and install the package using `remotes::install_github("jmclawson/stylo2gg")`.[^1]

[^1]: This notebook was created using R version 4.2.1 [@base] with the following R packages: ggforce v. 0.4.1 [@ggforce], knitr v. 1.42 [@knitr2014; @knitr2015; @knitr2023], rmarkdown v. 2.21 [@rmarkdown2018; @rmarkdown2020; @rmarkdown2023], stylo v. 0.7.5 [@stylo], stylo2gg v. 1.0 [@stylo2gg], tidyverse v. 2.0.0 [@tidyverse].
...
Here, the objects `my_data1` (created by `stylo::stylo()` before being logged by `stylo_log()`), `my_data2` (replicated by `stylo2gg::stylo_replicate()`), and `my_data3` (created and logged via `stylo_replicate()`) are almost identical. This makes it easier to keep track of the processes used in analysis, providing the "documentation mode" and "replication mode" Schöch calls for.
...
```{r}
#| fig-cap: 'Setting `viz=pca` rather than the stylo-flavored `viz="PCR"` or `viz="PCV"` prepares a minimal visualization of a principal components analysis derived from a correlation matrix. This might be a good setting to use if further customizing the figure by adding refinements provided by ggplot2 functions.'
...
Written by a user to address specific needs of users when visualizing with Stylo for research, Stylo2gg extends a beloved package. Over the past year, and especially because of suggestions proposed by others working with stylometry in R, it has added new features and become even more useful.
